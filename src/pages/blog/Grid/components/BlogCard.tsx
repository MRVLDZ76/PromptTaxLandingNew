...
const BlogCard = ({ blog }: { blog: BlogType }) => {
  const { category, image, publishedBy, title, description, slug } = blog
  const blogUrl = slug ? `/blog/${slug}` : "/blog/single/v1";
  ...
  return (
    <Card as={"article"} className="bg-transparent h-100 p-0">
      ...
      <CardBody className="px-2 pb-4">
        <CardTitle as={"h6"} className="mb-2"><Link to={blogUrl}>{title}</Link></CardTitle>
      ...
      <CardFooter className="bg-transparent d-flex justify-content-between px-2 py-0">
        ...
        <Link className="icon-link icon-link-hover stretched-link" to={blogUrl}>Read more<BsArrowRight className="bi" /> </Link>
      </CardFooter>
    </Card>
  )
}
